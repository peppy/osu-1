...
namespace osu.Game.Overlays.KeyBinding
{
    public abstract class KeyBindingsSubsection : SettingsSubsection
    {
        ...
        {
            var bindings = store.Query(Ruleset?.OnlineID, variant);
            ...
            foreach (var defaultGroup in Defaults.GroupBy(d => d.Action))
            {
                ...
                Add(new KeyBindingRow(defaultGroup.Key, bindings.Where(b => ((int)b.KeyBinding.Action).Equals(intKey)).Select(dkb => dkb.KeyBinding))
                {
