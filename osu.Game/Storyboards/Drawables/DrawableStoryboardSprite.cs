...
namespace osu.Game.Storyboards.Drawables
{
    public class DrawableStoryboardSprite : Sprite, IFlippable
    {
        ...
        {
            var path = beatmap.Value.BeatmapSetInfo?.Files?.FirstOrDefault(f => f.Filename.Equals(Sprite.Path, StringComparison.InvariantCultureIgnoreCase))?.FileInfo.StoragePath;
            if (path == null)
                return;
